...
class IdentityTest(parameterized.TestCase):
    def setUp(self):
        super().setUp()
        self.q_t = np.array([[[1.2, 2.2], [-1.2, 0.2], [2.2, -1.2]], [[4.2, 2.2], [1.2, 1.2], [-1.2, -2.2]]], dtype=np.float32)
    ...
    def test_identity(self, tx, inv_tx):
        ...
        q_t = torch.tensor(self.q_t)
        np.testing.assert_allclose(inv_tx(tx(q_t)), self.q_t, rtol=1e-3)
        np.testing.assert_allclose(tx(inv_tx(q_t)), self.q_t, rtol=1e-3)
...
#         self.q_tm1 = np.array(
#             [[[1.1, 2.1], [-1.1, 1.1], [3.1, -3.1]], [[2.1, 3.1], [-1.1, 0.1], [-2.1, -1.1]]], dtype=np.float32
#         )
#         self.a_tm1 = np.array([[0, 1, 0], [1, 0, 0]], dtype=np.int32)
#         self.discount_t = np.array([[0.0, 0.89, 0.85], [0.88, 1.0, 0.83]], dtype=np.float32)
...
#         transformed_q_lambda = self.variant(
#             jax.vmap(functools.partial(nonlinear_bellman.transformed_q_lambda, tx_pair=tx_pair, lambda_=self.lambda_))
#         )
#         # Compute vtrace output.
#         actual_td = transformed_q_lambda(self.q_tm1, self.a_tm1, self.r_t, self.discount_t, self.q_t)
#         # Test output.
...
#             [[[1.2, 2.2], [-1.2, 0.2], [2.2, -1.2]], [[4.2, 2.2], [1.2, 1.2], [-1.2, -2.2]]], dtype=np.float32
#         )
#         self.a_t = np.array([[0, 1, 0], [1, 0, 0]], dtype=np.int32)

#         self.expected_td = np.array(
#             [
#                 [[-2.4, 1.3112999, 1.0700002], [3.9199996, 2.104, -2.196]],
#                 [[-1.9329091, 0.9564189, -0.7853615], [-0.9021418, 1.1716722, 0.2713145]],
#                 [[-1.6178751, 0.85600746, -0.75762916], [-0.87689304, 0.6246443, 0.3598088]],
#                 [[-2.178451, 1.02313, -0.593768], [-0.415362, 1.790864, -0.120749]],
#             ],
#             dtype=np.float32,
#         )
...
#     def test_transformed_q_lambda_batch(self, tx_pair, td_index):
#         """Tests correctness for full batch."""
#         transformed_n_step_q_learning = self.variant(
#             jax.vmap(functools.partial(nonlinear_bellman.transformed_n_step_q_learning, tx_pair=tx_pair, n=self.n))
#         )
#         actual_td = transformed_n_step_q_learning(self.q_tm1, self.a_tm1, self.target_q_t, self.a_t, self.r_t, self.discount_t)
#         np.testing.assert_allclose(self.expected_td[td_index], actual_td, rtol=1e-3)
...
class TransformedRetraceTest(parameterized.TestCase):
    ...
    def test_transformed_retrace_batch(self, tx_pair, td_index):
        """Tests correctness for full batch."""
        transformed_retrace = functools.partial(nonlinear_bellman.transformed_retrace, tx_pair=tx_pair, lambda_=self._lambda)

        # Compute transformed vtrace td errors in batch.
        actual = transformed_retrace(
        ...
        # Test output.
        np.testing.assert_allclose(self.expected_td[td_index].swapaxes(0, 1), actual.extra.td_error.numpy(), rtol=1e-3)
